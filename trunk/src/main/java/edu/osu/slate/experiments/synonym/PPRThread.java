 ...
 *
 */
public class PPRThread extends Thread
{
  // Synonym Task File
  ...
  private VectPersonalizedPageRank ppr;
  ...
  */
  public PPRThread(TermToVertexMapping t2v, WikiGraph spr, String tFile, String rFile, String vFile)
  {
    term2Vertex = t2v;
    ppr = new VectPersonalizedPageRank(spr);
    taskFile = tFile;
    resultFile = rFile;
    vertexFile = vFile;
  }
      ...
      for(int x = 0; vcSource != null && x < vcSource.length; x++)
      {
        ...
        for(int i = 0; i < vcTerms.length; i++)
        {
          ...
          for(int y = 0; vcTerms[i] != null && y < vcTerms[i].length; y++)
          {
            ...
            double val = ppr.getExactRelatedness(from, currentVertex);
            
            /* Check to see:
             * - The vertex is valid
             * - Relatedness value is largest seen for the term 
             */
            if(currentVertex >= 0 && val > vals[i])
            {
