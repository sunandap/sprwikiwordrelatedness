...
public class CheckWordPairMappingCounts {
  ...
  /* */
  private static WordToVertexMapping word2VertexF;
  private static WordToVertexMapping word2VertexT;
  ...
  public static void main(String[] args)
  {
    ...
    try
    {
      ...
      word2VertexF = (WordToVertexMapping) in.readObject();
      in.close();
      word2VertexF.stem = false;
    }
    catch(Exception e)
    {
    ...
    {
      in = new ObjectInputStream(new FileInputStream(wordVertexMapFile));
      word2VertexT = (WordToVertexMapping) in.readObject();
      in.close();
      word2VertexT.stem = true;
    }
    catch(Exception e)
    {
      System.err.println("Problem with file: " + wordVertexMapFile);
    ...
    while(it.hasNext())
    {
      ...
      String word = it.next();
      String[] candidateWords = Common.resolve(word, word2VertexF);
      ...
      candidateWords = Common.resolve(word, word2VertexT);
    ...
    for(int i = 1; i <= length; i++)
    {
      ...
      {
        vals = vals.append( ((countsF.get(i)/totalWMF)*100) + ",");
      }
      else
      {
        vals = vals.append("0,");
      ...
      {
        vals = vals.append( ((countsT.get(i)/totalWMT)*100) + ";");
      }
      else
      {
        vals = vals.append("0;");
    ...
    for(int i = 1; i <= length; i++)
    {
      ...
      {
        vals = vals.append( ((mcCountsF.get(i)/totalMCF)*100) + ",");
      }
      else
      {
        vals = vals.append("0,");
      ...
      {
        vals = vals.append( ((mcCountsT.get(i)/totalMCT)*100) + ";");
      }
      else
      {
        vals = vals.append("0;");
