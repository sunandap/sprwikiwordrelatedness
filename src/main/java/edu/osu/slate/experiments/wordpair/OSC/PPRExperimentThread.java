 ...
 *
 */
public class PPRExperimentThread extends Thread
{
   ...
   */
   public PPRExperimentThread(TermToVertexMapping t2v, WikiGraph wg, String task, int part)
   {
     term2Vertex = t2v;
     ppr = new VectPersonalizedPageRank(wg);
     this.task = task;
     this.part = part;
     setFiles();
   }
    ...
    while(s.hasNext())
    {
      ...
      for(int x = 0; vc2 != null && x <vc2.length; x++)
      {
        ...
        for(int y = 0; vc1 != null && y <vc1.length; y++)
        {
          ...
          double relValue = ppr.getRelatedness(v1, v2);
          if(relValue > d21)
          {
            maxV21 = v1;
            maxV22 = v2;
